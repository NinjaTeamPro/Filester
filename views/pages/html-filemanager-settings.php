...
if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['njt-settings-form-submit'] ) ) {
	if ( ! wp_verify_nonce( $_POST['njt-fs-settings-security-token'], 'njt-fs-settings-security-token' ) ) {
		wp_die();
	}
...
<div class="njt-fs-settings njt-fs-file-manager">
  ...
  <div class="njt-fs-settings-content">
	<form action="" class="njt-plugin-setting settings-form" method="POST">
	  ...
	  <table class="form-table">
		  ...
		  <td>
			<textarea type='text' name='root_folder_path' id='root_folder_path' class="njt-settting-width"
			  placeholder="ex: <?php echo ( str_replace( '\\', '/', ABSPATH ) . 'wp-content' ); ?>"> 
										  <?php
											if ( isset( $this->options['njt_fs_file_manager_settings']['root_folder_path'] ) && ! empty( $this->options['njt_fs_file_manager_settings']['root_folder_path'] ) ) {
												echo str_replace( '\\', '/', esc_attr( $this->options['njt_fs_file_manager_settings']['root_folder_path'] ) );}
											?>
				</textarea>
			<div>
		...
		<tr>
		  ...
		  <td>
			<textarea type='text' name='root_folder_url' id='root_folder_url' class="njt-settting-width"
			  placeholder="ex: <?php echo ( site_url() ); ?>"> 
										  <?php
											if ( isset( $this->options['njt_fs_file_manager_settings']['root_folder_url'] ) && ! empty( $this->options['njt_fs_file_manager_settings']['root_folder_url'] ) ) {
												echo str_replace( '\\', '/', esc_attr( $this->options['njt_fs_file_manager_settings']['root_folder_url'] ) );}
											?>
				</textarea>
			<div>
			<p class="description njt-settting-width">
		...
		<tr>
		  ...
		  <td>
			<input type="number" name="upload_max_size" id="upload_max_size" class="njt-fs-settting-width-half"
			  value="
			  <?php
				if ( isset( $this->options['njt_fs_file_manager_settings']['upload_max_size'] ) && ! empty( $this->options['njt_fs_file_manager_settings']['upload_max_size'] ) ) {
					echo esc_attr( $this->options['njt_fs_file_manager_settings']['upload_max_size'] );}
				?>
				">
		...
		<tr>
		  ...
		  <td>
			<?php require_once $viewPathLanguage; ?>
		  </td>
		</tr>
		<!-- .htaccess -->
		<tr>
	...
	<?php require_once $viewUserRoleRestrictions; ?>
  </div>
</div>
